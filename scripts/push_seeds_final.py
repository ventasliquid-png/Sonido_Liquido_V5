...
def push_to_cloud():
    ...
    try:
        ...
        # 1. Push Clientes
        path_cli = os.path.join(DATA_DIR, "CLIENTES_MAESTRO_LATEST.csv")
        if os.path.exists(path_cli):
            ...
            if not df_cli.empty:
                ...
                print("⚠ CLIENTES_MAESTRO_LATEST.csv está vacío.")
        else:
             print("⚠ No se encontró CLIENTES_MAESTRO_LATEST.csv")

        # 2. Push Productos
        path_prod = os.path.join(DATA_DIR, "PRODUCTOS_MAESTRO_LATEST.csv")
