...
# --- FIX: Cargar Contexto de App para SQLAlchemy Registry ---
import backend.main 
...
def war_game_v3():
    print("\n--- 🛡️ WAR GAME 3.0: PRICING ENGINE VERIFICATION (FIXED) 🛡️ ---")
    ...
    try:
        ...
        # Check if exists to avoid unique constraint error if run multiple times (though rollback handles it usually)
        
        prod_std = Producto(nombre="TEST_STD", rubro_id=1, sku=99001, descripcion="Standard")
        db.add(prod_std)
        db.flush()
        ...
        prod_ovr = Producto(nombre="TEST_OVERRIDE", rubro_id=1, sku=99002, descripcion="Override")
        db.add(prod_ovr)
        db.flush()
        ...
        print("\n✅ VICTORIA EN WAR GAME 3.0: SISTEMA DE PRECIOS VALIDADO.")
