...
const handleDomicilioSaved = async (domicilioData) => {
    try {
        ...
        if (domicilioData.linked_delivery_id) {
            ...
            // [GY-FIX] For the separate delivery node, we MUST map the delivery fields to the 
            // main address fields (calle, numero, etc.) so they are saved as the actual address
            // of that node.
            deliveryPayload.calle = domicilioData.calle_entrega;
            deliveryPayload.numero = domicilioData.numero_entrega;
            deliveryPayload.piso = domicilioData.piso_entrega;
            deliveryPayload.depto = domicilioData.depto_entrega;
            deliveryPayload.cp = domicilioData.cp_entrega;
            deliveryPayload.localidad = domicilioData.localidad_entrega;
            deliveryPayload.provincia_id = domicilioData.provincia_entrega_id;
