// [IDENTIDAD] - frontend\src\views\Maestros\TipoContactoForm.vue
...
watch(() => props.show, (newVal) => {
    if (newVal) {
        if (props.id) {
            ...
            const tipo = store.tiposContacto.find(t => t.id === props.id);
            if (tipo) {
                form.id = tipo.id;
...
const handleSave = async () => {
    try {
        let result;
        if (isEditing.value) {
            result = await store.updateTipoContacto(props.id, { nombre: form.nombre });
        } else {
            result = await store.createTipoContacto(form);
        }
        emit('saved', result);
        emit('close');
    } catch (error) {
        alert('Error al guardar el tipo de contacto.');
        console.error(error);
    }
};
...
<template>
    <div v-if="show" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-[60] flex items-center justify-center">
        <div class="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white animate-scale-in">
            <div class="mt-3">
                <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">{{ isEditing ? 'Editar Tipo de Contacto' : 'Nuevo Tipo de Contacto' }}</h3>
