                            ...
                            <Teleport to="body">
                                ...
                                <div v-if="showContextMenu"
                                    ...
                                    <div class="flex justify-between items-center px-2 pb-1 mb-1 border-b border-[#454545] text-xs text-gray-500">
                                        <span class="font-bold">Acciones de Cliente</span>
                                    ...
                                    <div v-if="clienteSeleccionado && (clienteSeleccionado.id || clienteSeleccionado._id)"
                                        ...
                                        <span>Modificar / Ver Ficha</span>
                                    </div>
                                    <div v-else class="px-3 py-1.5 text-gray-400 italic cursor-default">
                                        Sin cliente seleccionado
                                    </div>
                                    <div @click="altaClienteContext"
                                         class="px-3 py-1.5 hover:bg-[#414141] cursor-default flex items-center gap-3 text-white transition-colors border-t border-[#454545] mt-1">
                                        <i class="fas fa-plus-circle text-emerald-500 text-xs"></i>
                                        <span>Alta de Cliente (F4)</span>
                                    </div>
                                </div>
                            </Teleport>
                    ...
                    <div class="shrink-0 p-2 relative z-[60]">
                        <div class="grid grid-cols-12 px-4 py-4 gap-2 bg-emerald-500/5 rounded-lg items-center border border-emerald-500/20 shadow-lg relative min-h-[70px]">
                            ...
                            <div class="col-span-4 relative">
                                ...
                                >
                                <!-- PRODUCT CONTEXT MENU TELEPORT -->
                                <Teleport to="body">
                                    <div v-if="showProductContextMenu" class="fixed inset-0 z-[9998]" @click="showProductContextMenu = false" @contextmenu.prevent></div>
                                    <div v-if="showProductContextMenu"
                                         :style="{ top: productContextMenuY + 'px', left: productContextMenuX + 'px' }"
                                         class="fixed z-[9999] bg-[#2b2b2b] border border-[#454545] shadow-lg py-1 w-64 select-none flex flex-col text-[13px] font-sans rounded-none"
                                         @click.stop
                                         @mousedown.stop
                                         @contextmenu.prevent>
                                        <div class="flex justify-between items-center px-2 pb-1 mb-1 border-b border-[#454545] text-xs text-gray-500">
                                            <span class="font-bold">Acciones de Producto</span>
                                            <i @click="showProductContextMenu = false" class="fas fa-times hover:text-white cursor-pointer p-1"></i>
                                        </div>
                                        <div v-if="newItem.producto_obj"
                                             @click="irAFichaProducto"
                                             class="px-3 py-1.5 hover:bg-[#414141] cursor-default flex items-center gap-3 text-white transition-colors">
                                            <i class="fas fa-external-link-alt text-emerald-400 text-xs"></i>
                                            <span>Modificar / Ver Ficha</span>
                                        </div>
                                        ...
                                        <div @click="altaProductoContext"
                                             class="px-3 py-1.5 hover:bg-[#414141] cursor-default flex items-center gap-3 text-white transition-colors border-t border-[#454545] mt-1">
                                            <i class="fas fa-plus-circle text-emerald-500 text-xs"></i>
                                            <span>Alta de Producto (F4)</span>
                                        </div>
                                    </div>
                                </Teleport>
                    ...
                    <div class="text-right pl-6 border-l border-white/10 flex flex-col justify-end gap-2">
                         ...
                         <div class="flex items-baseline gap-2 justify-end mb-2">
                              ...
                              <button @click="savePedido(false)" 
                                      ...
                                      :class="['disabled:bg-gray-700 disabled:cursor-not-allowed text-white font-bold py-3 px-6 rounded-lg shadow-lg transition-all flex items-center justify-center gap-2 uppercase tracking-wider text-sm', isCircuitoNegro ? 'bg-pink-500 hover:bg-pink-400 hover:shadow-pink-500/20' : 'bg-emerald-500 hover:bg-emerald-400 hover:shadow-emerald-500/20']">
...
const productContextMenuX = ref(0);
const productContextMenuY = ref(0);
...
const handleProductContextMenu = (event) => {
    ...
    productContextMenuX.value = event.clientX;
    productContextMenuY.value = event.clientY;
    showProductContextMenu.value = true;
};

const irAFichaProducto = () => {
    const p = newItem.value.producto_obj;
    if (p) {
        const { href } = router.resolve({ 
            name: 'Productos', 
            query: { search: p.sku || p.nombre, mode: 'satellite' }
        });
        ...
        window.open(href, 'EdicionProducto', features);
        showProductContextMenu.value = false;
    }
};

const altaProductoContext = () => {
    const searchTerm = newItem.value.sku || newItem.value.descripcion;
    const { href } = router.resolve({ 
        name: 'Productos', 
    ...
    window.open(href, 'AltaProductoContext', features);
    showProductContextMenu.value = false;
};
...
const filteredClientes = computed(() => {
    ...
    const results = clientesStore.clientes.filter(c => {
        if (c.activo === false) return false; // Safety filter
...
const handleGlobalKeys = (e) => {
    if (e.key === 'F3') {
        ...
        // Contextual F3:
        // If in Header Area (Client/OC/Date) -> Focus Client Input
        // If in Items Area or Footer -> Focus Product Desc Input
        
        const headerElements = [
            clientInputRef.value,
            // Assuming other inputs in header don't have refs yet, we check for presence in header 
            document.querySelector('.col-span-2 input[type="date"]'),
            document.querySelector('.col-span-1 input[placeholder="---"]') 
        ];

        if (headerElements.includes(active) || !active || active === document.body) {
            clientInputRef.value?.focus();
            showClienteResults.value = true; // Show dropdown
        } else {
            inputDescRef.value?.focus();
            showProductResults.value = true; // Show dropdown
        }
    }
...
const handleMessage = async (event) => {
    ...
    if (event.data?.type === 'PRODUCTO_CREADO') {
        console.log('[PedidoCanvas] Received signal: New product created in satellite.');
        // Refresh product list from server to include the newly created item
        await productosStore.fetchProductos();
        
        // If we have a specific name, we could theoretically select it
        if (event.data.producto?.nombre) {
            notificationStore.add(`Producto "${event.data.producto.nombre}" sincronizado.`, 'success');
        }
    }
    
    if (event.data?.type === 'CLIENTE_CREADO') {
        console.log('[PedidoCanvas] Received signal: New client created in satellite.');
        await clientesStore.fetchClientes();
        if (event.data.cliente?.razon_social) {
            notificationStore.add(`Cliente "${event.data.cliente.razon_social}" sincronizado.`, 'success');
        }
    }
};
