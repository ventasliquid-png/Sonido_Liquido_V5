          ...
          <div class="flex flex-col gap-2">
              ...
              >
                  ...
                  <div class="w-32 flex flex-col leading-tight">
                      ...
                      <span class="text-[10px] text-emerald-300/70 font-mono">{{ formatDate(pedido.fecha).split(' ')[1] }}</span>
                  </div>
...
const exportExcel = async () => {
    try {
        notificationStore.add('Generando Excel de Seguridad...', 'info')
        ...
        a.href = url
        // Filename is in content-disposition but let's trust backend default or browser
        // Or extract:
        const cp = response.headers.get('Content-Disposition')
        let filename = 'SafetyNet_Pedidos.xlsx'
        if (cp && cp.includes('filename=')) {
            filename = cp.split('filename="')[1].split('"')[0]
        }
        ...
        notificationStore.add('Exportación completada', 'success')
    } catch (e) {
        console.error(e)
        notificationStore.add('Error al exportar Excel', 'error')
    }
}
</script>
