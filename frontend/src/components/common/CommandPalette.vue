  ...
  <div v-if="show" class="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh] px-4">
    ...
    <div class="relative w-full max-w-lg bg-[#1a1f2e] border border-white/10 rounded-xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="flex items-center px-4 py-4 border-b border-white/10">
            <i class="fas fa-search text-cyan-400 text-xl mr-4 animate-pulse"></i>
            <input 
                ref="searchInput"
                v-model="query"
                type="text" 
                class="flex-1 bg-transparent border-none outline-none text-white placeholder-gray-500 text-xl font-outfit"
            ...
            />
            <div class="text-[10px] font-bold text-gray-600 border border-gray-700 rounded px-2 py-0.5 tracking-wider">ESC</div>
        </div>
        ...
        <div class="max-h-[60vh] overflow-y-auto py-2">
            ...
            <div v-else>
                <div v-for="(group, groupName) in groupedCommands" :key="groupName">
                    <div class="px-4 py-2 text-[10px] font-bold text-cyan-500/50 uppercase tracking-widest sticky top-0 bg-[#1a1f2e]/95 backdrop-blur z-10 border-b border-white/5">
                        {{ groupName }}
                    </div>
                    <button
                        v-for="cmd in group"
                        :key="cmd.id"
                        class="w-full flex items-center px-4 py-3 text-left transition-colors group relative border-l-2 border-transparent"
                        :class="{ 'bg-cyan-900/10 border-cyan-500': selectedIndex === cmd.index, 'hover:bg-white/5': selectedIndex !== cmd.index }"
                        @click="executeCommand(cmd)"
                        @mousemove="selectedIndex = cmd.index"
                    >
                        <!-- Selection Indicator -->
                        <div class="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-cyan-400 to-blue-500 transition-opacity" :class="{ 'opacity-100': selectedIndex === cmd.index, 'opacity-0': selectedIndex !== cmd.index }"></div>
                        
                        <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-white/5 mr-4 group-hover:bg-white/10 transition-colors shadow-inner"
                             :class="getIconColorClass(cmd.group)"
                        >
                            <i :class="cmd.icon"></i>
                        </div>
                        <div class="flex-1">
...
const getIconColorClass = (group) => {
    switch(group) {
        case 'Acciones Rápidas': return 'text-cyan-400'
        case 'Navegación': return 'text-blue-400'
        case 'Logística': return 'text-amber-400'
        case 'Maestros': return 'text-purple-400'
        case 'Operativo': return 'text-emerald-400'
        case 'Agenda': return 'text-pink-400'
        case 'Sistema': return 'text-red-400'
