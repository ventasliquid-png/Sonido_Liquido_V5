...
### 2025-12-15 13:10 - Protocolo de Sincronizaci�n de Datos (Anti-Desliz)
> [!IMPORTANT]
> **LECCI�N APRENDIDA:** El c�digo viaja por Git, pero los **DATOS** (SQLite) viajan por **Semillas (CSV)**.
>
> **Problema:** Se trabaj� en una ubicaci�n (CA), se agregaron clientes/productos, pero no se exportaron las semillas. Al llegar a la otra ubicaci�n (OF), el c�digo estaba actualizado pero la base de datos vac�a o desactualizada.
> **Intento Fallido:** Se intent� conectar a la Nube (IOWA) como backup, pero fall� por timeout (Firewall/IP).

**PROTOCOLO OBLIGATORIO AL TERMINAR SESI�N (HANDOVER):**
...
---
## [SESI�N 17-DIC-2025] - Limpieza Profunda y Acceso LAN

### Objetivo
Habilitar herramientas para la 'depuraci�n masiva' de la base de datos (clientes/productos basura) y configurar el entorno para permitir la carga de datos concurrente desde m�ltiples puestos (LAN).

### Logros
1.  **Limpieza Masiva (Bulk Hard Delete):**
    *   Implementado mecanismo de 'Doble Fase':
        *   **Fase 1 (Naranja):** Desactivaci�n masiva (Soft Delete) desde el filtro 'Activos'.
        *   **Fase 2 (Rojo):** Eliminaci�n definitiva (Hard Delete) desde el filtro 'Inactivos'.
...
2.  **Sanitizaci�n de CUITs:**
    *   **Frontend:** El inspector limpia autom�ticamente guiones y espacios al guardar.
    *   **Backend:** Validator Pydantic ('clean_cuit') fuerza string num�rico de 11 d�gitos.
    *   Resultado: Se permite input flexible ('20-1234...') pero se guarda limpio ('201234...').

3.  **Acceso Remoto (LAN / Multijugador):**
    *   **Configuraci�n Backend:** CORS habilitado para '*' (Wildcard).
    *   **Configuraci�n Firewall:** Scripts para abrir puertos 8000 (API) y 5173 (Web).
    *   **Hardcoding de IP:** Se fij� la IP '192.168.0.34' en 'api.js' y 'Login.vue' para anular problemas de resoluci�n de nombres o variables de entorno.
    *   **Launcher:** Se cre� 'scripts/run_lan.ps1' para automatizar el arranque en modo red.
    *   **Usuarios:** Se cre� el usuario 'operador' (carga restringida) y se resete� 'admin'.

### Estado Final
El sistema est� desplegado en la red local y listo para que el equipo comience la carga y depuraci�n de datos.
