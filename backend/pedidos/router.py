# [IDENTIDAD] - backend\pedidos\router.py
...
# --- SAFETY NET: EXCEL EXPORT (V5.6) ---
from backend.pedidos.services.excel_export import ExcelExportService
from backend.tools.backup_system import BackupManager

@router.get("/export/excel")
def export_pedidos_excel(db: Session = Depends(get_db)):
    """
    Exporta TODOS los pedidos activos a Excel.
    Safety Net para operación autónoma with SILENT SNAPSHOT.
    """
    try:
        # Trigger Manual: Snapshot antes de exportar
        BackupManager.perform_backup(prefix="manual_excel_creation", silent=True)
        
        service = ExcelExportService()
        buffer = service.generate_orders_excel(db)
        
        filename = f"V5_Pedidos_SafetyNet_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error exporting safety net: {str(e)}")
# ---------------------------------------
