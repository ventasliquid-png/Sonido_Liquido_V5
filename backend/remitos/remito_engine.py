...
class PDFRemito(FPDF):
    ...
    def footer(self):
        # [SABUESO ORO] Bloque de Validación ARCA (30mm altura)
        # Solo se imprime si hay datos de CAE/QR
        if (self.qrcode_url or self.factura_vinculada) and self.cae:
            original_auto_page_break = self.auto_page_break
            self.set_auto_page_break(False)

            # --- CONFIGURACIÓN ---
            Y_START = 267      
            QR_SIZE = 25       # 2.5x2.5 cm
            MARGIN_X = 10      
            ...
            # Basado en Protocolo Sabueso Oro (RAR)
            ...
            # 2. CAE N°
            self.set_font('Arial', 'B', 9)
            self.set_xy(X_POS_RIGHT, Y_START + 8)
            self.cell(CELL_W, 5, f"CAE N°: {self.cae}", border=0, align='R')
            
            # 3. Vto. CAE
            self.set_xy(X_POS_RIGHT, Y_START + 13)
            self.cell(CELL_W, 5, f"Vto. CAE: {self.vto_cae or 'N/A'}", border=0, align='R')
            
            # 4. Comprobante Autorizado
            self.set_xy(X_POS_RIGHT, Y_START + 21)
            self.set_font('Arial', 'I', 8)
            self.set_text_color(100, 100, 100)
            self.cell(CELL_W, 4, "Documento de Transporte amparado por Factura Electrónica.", border=0, align='R')
    ...
    def add_content(self, cliente_data, items):
        ...
        # and it's called by FPDF on each add_page(). Emptying add_content overlap.
        pass
